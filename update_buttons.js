...
try {
  ...
  html = html.replace(
    '<a href="#kontak" class="btn btn-outline" style="font-size:0.75rem; padding:8px; margin-bottom:8px; display:block;" data-t="price_a_cta_random">+ Domain Perusahaan (Total 149K)</a>',
    '<a href="javascript:void(0)" onclick="toggleBundle(\'a\', 99, 50, this)" class="btn btn-bundle" style="font-size:0.75rem; padding:8px; margin-bottom:8px; display:block; background:transparent; color:#dc2626; border:2px solid #dc2626;" data-t="price_a_cta_random">+ Domain Perusahaan (50K)</a>'
  );
  html = html.replace(
  ...
  html = html.replace(
    '<a href="#kontak" class="btn btn-dark" style="font-size:0.75rem; padding:8px; margin-bottom:8px; display:block; background:#fff; color:#000; border:2px solid #000;" data-t="price_b_cta_random">+ Domain Perusahaan (Total 300K)</a>',
    '<a href="javascript:void(0)" onclick="toggleBundle(\'b\', 250, 50, this)" class="btn btn-bundle" style="font-size:0.75rem; padding:8px; margin-bottom:8px; display:block; background:transparent; color:#dc2626; border:2px solid #dc2626;" data-t="price_b_cta_random">+ Domain Perusahaan (50K)</a>'
  );
  html = html.replace(
  ...
  html = html.replace(
    '<a href="#kontak" class="btn btn-outline" style="font-size:0.75rem; padding:8px; margin-bottom:8px; display:block;" data-t="price_c_cta_random">+ Domain Perusahaan (Total 700K)</a>',
    '<a href="javascript:void(0)" onclick="toggleBundle(\'c\', 650, 50, this)" class="btn btn-bundle" style="font-size:0.75rem; padding:8px; margin-bottom:8px; display:block; background:transparent; color:#dc2626; border:2px solid #dc2626;" data-t="price_c_cta_random">+ Domain Perusahaan (50K)</a>'
  );
  html = html.replace(
...
<script>
  ...
  ['assets/js/script.src.js', 'assets/js/script.js'].forEach(f => {
    ...
    c = c.replace(/\+ Domain Perusahaan \(Total 149K\)/g, '+ Domain Perusahaan (50K)');
    c = c.replace(/\+ Domain \.com\/\.id \(Total 399K\)/g, '+ Domain .com/.id/.net (300K)');
    
    c = c.replace(/\+ Domain Perusahaan \(Total 300K\)/g, '+ Domain Perusahaan (50K)');
    c = c.replace(/\+ Domain \.com\/\.id \(Total 550K\)/g, '+ Domain .com/.id/.net (300K)');
    
    c = c.replace(/\+ Domain Perusahaan \(Total 700K\)/g, '+ Domain Perusahaan (50K)');
    c = c.replace(/\+ Domain \.com\/\.id \(Total 950K\)/g, '+ Domain .com/.id/.net (300K)');

    c = c.replace(/\+ Company Domain \(Total 149K\)/g, '+ Company Domain (50K)');
    c = c.replace(/\+ \.com\/\.id Domain \(Total 399K\)/g, '+ .com/.id/.net Domain (300K)');
    
    c = c.replace(/\+ Company Domain \(Total 300K\)/g, '+ Company Domain (50K)');
    c = c.replace(/\+ \.com\/\.id Domain \(Total 550K\)/g, '+ .com/.id/.net Domain (300K)');
    
    c = c.replace(/\+ Company Domain \(Total 700K\)/g, '+ Company Domain (50K)');
